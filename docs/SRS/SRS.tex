...
\begin{itemize}
    ...
    \item \textbf{Radiologists}: Medical doctors specializing in diagnosing and treating conditions using medical imaging (e.g., X-rays, CT, MRI).
    ...
    \item \textbf{TorchXRAY Vision}: An open-source library for classifying diseases based on chest X-ray images, offering pre-trained models to accelerate the development process.
    ...
    \item \textbf{Accuracy Rate}: Refers to the percentage of correct predictions made by the AI model when classifying chest X-rays.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Batch Size:} 32 images per batch – Defines the number of images processed together during each training iteration.
    \item \textbf{Learning Rate:} 0.001 – The step size during gradient descent, impacting model training speed.
    \item \textbf{Number of Epochs:} 50 – The number of times the model processes the entire training dataset.
    \item \textbf{Input Image Size:} 224x224 pixels – Suitable for CNN architectures, balancing computational efficiency and image detail.
    \item \textbf{Threshold for ROC Analysis:} 0.85 – The probability cut-off for positive classification, balancing sensitivity and specificity.
    \item \textbf{Precision:} 92\% – Reflects the percentage of true positive cases among all cases predicted positive.
    \item \textbf{Recall:} 88\% – Reflects the percentage of true positive cases among all actual positive cases.
\end{itemize}
...
\item[A\refstepcounter{assumpnum}\theassumpnum \label{A_meaningfulLabel}:] ML Models validation process will have to be \textbf{publicly published and accepted by all stakeholders} after being developed. Reason being the accuracy of the ML model is vital for safety of the patient, as well as the sucesss of the project, therefore this validation method need to be transparent to all the stakeholders, and users.
\item[A\refstepcounter{assumpnum}\theassumpnum \label{A_meaningfulLabel}:] AWS will share the responsibility with Neuralanalyzer to protect the customer PHI in a law-compliance manner, as mentioned in this \href{https://docs.aws.amazon.com/whitepapers/latest/aws-risk-and-compliance/shared-responsibility-model.html}{share-responsibility blog post}.
...
\subsection{Health and Safety Impact}
...
\subsection{Overall Societal Impact}
