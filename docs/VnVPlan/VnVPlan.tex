...
% Define placeholder commands
\newcommand{\progname}{Neuralanalyzer}
\newcommand{\authname}{Team Name}
\newcommand{\srsurl}{https://github.com/RezaJodeiri/CXR-Capstone/blob/main/docs/SRS/SRS.pdf}
...
April 4 & 2.0 & Final update and revision \\
\bottomrule
\end{tabularx}
...
The three main relevant documentation that helped guide us in System Verification and Validation Plan (VnV) was \href{https://github.com/RezaJodeiri/CXR-Capstone/blob/main/docs/DevelopmentPlan/DevelopmentPlan.pdf}{Development Plan}, \href{\srsurl}{Software Requirements and Specification}, and 
...
\begin{itemize}
  ...
  \begin{itemize}
    \item Individual units of the software are tested in isolation from the rest of the system. This ensures that each unit functions as expected based on the business use-cases.
    \item Examples of unit tests include: testing input validation, output of a function, button clicks, and authenticated web pages.
    \item All unit tests must cover both the happy path and edge cases (boundary conditions, invalid input, etc.).
  \end{itemize}
  
  \item[] \textbf{Integration Testing}
  \begin{itemize}
    \item The integration between two or more system components is tested to ensure they work together as designed in the business use-cases.
    \item In the context of Neuralanalyzer, required integrations to test include: the front-end and the back-end, back-end with AWS Cognito, back-end with the Neuralanalyzer ML model, and back-end with TorchXRayVision.
    \item Examples of integration tests include: testing back-end responses with various inputs from the front-end, or testing ML predictions with different image inputs.
    \item Integration tests should ideally isolate components (e.g., front-end to back-end tests should mock the ML model's responses).
    \item All integration tests must cover both happy path and edge cases (boundary conditions, invalid input, etc.).
  \end{itemize}

  \item[] \textbf{System / End-to-End Testing}
  \begin{itemize}
    \item System testing ensures that all components work together as designed in the business use-cases.
    \item Example test case: signing up on the front-end and validating the user on the back-end.
    \item All end-to-end tests must cover all business use-cases (login, sign-up, image upload, prediction), and functional requirements.
  \end{itemize}

  \item[] \textbf{Periodic Health Checks}
  \begin{itemize}
    \item The front-end, back-end, and ML model will be checked periodically to measure system uptime. Measured uptime must meet the agreed-upon SLA (95\%).
    \item Periodic health checks will be performed regardless of the software version or system state.
    \item The following performance parameters will be monitored:
    \begin{itemize}
      \item API response time must average under 500 milliseconds.
      \item System memory usage must stay below 80\% during normal operations.
      \item CPU usage must remain below 75\% under standard load.
      \item 95\% of database queries must respond within 1 second.
    \end{itemize}
  ...
  \begin{itemize}
    \item Manual testing will be performed by the team to ensure intuitive and simple UI/UX for end-users.
    \item It will include interaction with the front-end on various devices (mobile, tablet, desktop).
    \item All manual tests must cover all business use-cases (login, sign-up, image upload, prediction).
  \end{itemize}

  \item[] \textbf{Machine Learning Model Validation and Testing}
  \begin{itemize}
    \item All Chest X-Ray data will be split into three distinct sets: training, validation, and testing.
    \item The model will be trained on the training set and validated on the validation set.
    \item The model will be tested on the test set, and accuracy will be measured. The target accuracy will be agreed upon by the team and stakeholders.
  \end{itemize}

  \item[] \textbf{User Acceptance Testing}
  \begin{itemize}
    \item After the MVP is ready, selected stakeholders will be invited to test the system.
    \item Based on feedback, the team will make necessary changes using GitHub Issues.
  \end{itemize}
...
\end{enumerate}
\subsubsection{\texorpdfstring{\sout{AI Model Inference Tests}}{AI Model Inference Tests}}

\sout{This subsection covers Requirement \#3 of the \href{https://github.com/RezaJodeiri/CXR-Capstone/blob/main/docs/SRS/SRS.pdf}{SRS}
document by testing that the system analyzes chest X-ray images using a convolutional neural network (CNN)-based AI model to detect the presence or absence of specific diseases with an accuracy of 75\% or higher.}\citep{SRS}
...
\subsubsection{\texorpdfstring{\sout{Role Based Access Control Tests}}{Role Based Access Control Tests}}

\sout{This subsection covers Requirement \#10 of the \href{https://github.com/RezaJodeiri/CXR-Capstone/blob/main/docs/SRS/SRS.pdf}{SRS}
document by testing that the system enforces role-based access control, ensuring users only access permitted features.} \citep{SRS}
...
\subsubsection{\texorpdfstring{\sout{Cultural}}{Cultural}}

\sout{This subsection's tests cover all Cultural requirements listed in the \href{https://github.com/RezaJodeiri/CXR-Capstone/blob/main/docs/SRS/SRS.pdf}{SRS}
 by ensuring that the system supports both English and French languages throughout the interface and reports, with no untranslated or misaligned content.}\citep{SRS}
