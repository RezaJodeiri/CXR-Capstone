...
\begin{itemize}
    ...
    \item With many moving parts of the application, our Proof of Concept Demo Plan has critical component and tasks that are considered risky if not implemented correctly. The risks are:
        \begin{itemize}
            \item Unable to confidently identify the disease with the open-source model.
            \item Unable to host the application on cloud environment (Fallback plan: hosting the application locally for demo).
            \item Unable to have access, and gather enough data to test the open-source ML Model.
            \item Unable to complete CI/CD pipeline to deploy app on cloud environment (Fallback plan: Deployed application manually).
        \end{itemize}
    \item To mitigate the risks mentioned above, our team had came up with fallback plans:
        \begin{itemize}
            \item If unable to confidently identify the disease with the TorchXRayVision model, we can fallback for another models.
            \item If unable to gather enough data for the model, we can alter the dataset and make multiple mutations of the x-ray images.
            \item If unable to host the application on cloud environment, we can host the application locally for the demo
            \item If unable to complete CI/CD pipeline to deploy app on cloud environment, we can deploy the application manually.
        \end{itemize}
