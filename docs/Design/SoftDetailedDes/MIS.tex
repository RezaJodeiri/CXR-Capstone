...
\subsubsection{HandleRequest(HTTP request)}
\begin{itemize}
  \item \textbf{Transition}:  Processes the incoming HTTP request and routes it to the appropriate view module.
  \item \textbf{Output}: Returns the HTTP response based on the request.
\end{itemize}
\subsubsection{Routes(user Action)}
\begin{itemize}
  \item \textbf{Transition}: Accepts user interaction or actions.
  \item \textbf{Output}: Produces the corresponding HTML for rendering.
\end{itemize}
...
\subsection{State Variables}
\begin{itemize}
  \item \textbf{isEditing}: A state variable that tracks whether the user is currently editing.
  \item \textbf{ReportDetails}: Contains the information associated with the reports.
\end{itemize}
...
\begin{tabular}{|l|l|l|l|}
    ...
    \textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
    \hline 
    \texttt{ViewReport} & ReportDetails:  & place holder & \texttt{place holder} \\
    \hline
\end{tabular}
...
\begin{tabular}{|l|l|l|l|}
    ...
    \textbf{Name} & \textbf{In} & \textbf{Out} & \textbf{Exceptions} \\
    \hline
    \texttt{predictDisease} & XrayImage: binary & Prediction Result: \newline PredictionResult & \texttt{PredictionFailedException} \\
    \hline
\end{tabular}
